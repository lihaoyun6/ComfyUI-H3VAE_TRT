            ...
            for j in range(split_count):
                ...
                if j == 0:
                    if dec_overlap is not None:
                        chunk = self.blend(
                            dec_overlap, chunk, self.frame_overlap, dim=-3
                        )
                        dec_overlap = None
    ...
    def encode(self, x):
        ...
        try:
            ...
            if x.shape[2] == 1:
                ...
                moments = self.tiled_encode(self._normalize_pixels(x_pad))[
                    :, :, -1:, :, :
                ]
            else:
...
class MiniMaxH3TRTVAELoader:
    ...
    def load_vae(self, decoder, encoder):
        ...
        vae_instance = MiniMaxH3TRTVAE(
            decoder_runner=dec_runner, encoder_runner=enc_runner
        )
...
class MiniMaxH3TRTCompilerNode:
    ...
    def _build_engine(cls, onnx_path, engine_path, is_decoder=True):
        if not HAS_TRT:
            raise RuntimeError(
                "TensorRT library not found! Please install with pip first."
            )
